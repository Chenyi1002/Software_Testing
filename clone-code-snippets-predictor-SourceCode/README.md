...
当前仓库主要包含研究用数据处理脚本及已有数据，尚未包含完整的缺陷分类模型训练、推理服务和统一流水线入口。本文根据版本 `e53ac3008e681c113b6123189caea1779933b121` 的源码整理，所列操作需在配置好本地路径和数据后执行；完整流程尚未按本文重新验证。
...
本项目可作为课程实践的被测对象，建议先将范围限定在以下三个模块：

| 模块 | 关注的功能范围 |
| --- | --- |
| `Extract_CloneCode_Snippets.py` | XML 解析、代码区间合并、片段数据写入 |
| `Extract_HistoryMetrics&Label.py` | 差异信息提取、历史统计、区间判断、标签处理 |
| `Combine.py` | 多表数据读取、区间合并、结果汇总 |

课程实践应保留原始被测版本，逐步提交需求说明、测试用例、测试脚本、执行记录、缺陷修复及验证结果。每位成员使用个人账号提交实际完成的工作，并在报告中说明贡献。

当前尚未建立课程自动化测试工程。完成后需在本节补充实际可执行的一键运行命令、测试依赖、数据准备方式和结果位置。

课程模块一要求独立设计用例并开展自动化测试，不使用 AI 生成用例或开展自动化测试；模块二可选择对本工具开展 AI 辅助测试，并记录 AI 参与过程、人工审查和局限分析。项目数据中的 `Bug` 标签与课程缺陷清单不同：后者应记录本工具自身经复现确认的问题。
